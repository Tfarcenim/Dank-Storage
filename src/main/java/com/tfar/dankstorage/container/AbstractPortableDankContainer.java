...
import com.tfar.dankstorage.network.MessageDankSlotContents;
...
public class AbstractPortableDankContainer extends Container {
  ...
  public AbstractPortableDankContainer(ContainerType<?> type, int p_i50105_2_, PlayerInventory playerInventory, PlayerEntity player, int rows, int stacklimit) {
    ...
    this.handler = new PortableDankHandler(9 * rows,stacklimit,bag);
    addOwnSlots(rows);
  ...
  public ItemStack slotClick(int slotId, int dragType, ClickType clickTypeIn, PlayerEntity player) {
    ...
    if (clickTypeIn == ClickType.QUICK_CRAFT) {
      ...
      if ((j1 != 1 || this.dragEvent != 2) && j1 != this.dragEvent) {
        ...
        if (slot7 != null && AbstractPortableDankContainer.canAddItemToSlot(slot7, mouseStack, true) && slot7.isItemValid(mouseStack) && (this.dragMode == 2 || mouseStack.getCount() > this.dragSlots.size()) && this.canDragIntoSlot(slot7)) {
        ...
        if (!this.dragSlots.isEmpty()) {
          ...
          for (Slot dragSlot : this.dragSlots) {
            ...
            if (dragSlot != null && AbstractPortableDankContainer.canAddItemToSlot(dragSlot, mouseStack, true) && dragSlot.isItemValid(mouseStack) && (this.dragMode == 2 || mouseStack.getCount() >= this.dragSlots.size()) && this.canDragIntoSlot(dragSlot)) {
      ...
      if (!mouseStack.isEmpty() && (slot == null || !slot.getHasStack() || !slot.canTakeStack(player))) {
        ...
        for (int k = 0; k < 2; ++k) {
          for (int l = i; l >= 0 && l < this.inventorySlots.size() && mouseStack.getCount() < mouseStack.getMaxStackSize(); l += j) {
            ...
            if (slot1.getHasStack() && AbstractPortableDankContainer.canAddItemToSlot(slot1, mouseStack, true) && slot1.canTakeStack(player) && this.canMergeSlot(mouseStack, slot1)) {
