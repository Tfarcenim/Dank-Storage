...
public abstract class AbstractAbstractDankStorageScreen<T extends AbstractAbstractDankContainer> extends ContainerScreen<T> {
	...
	protected void renderTooltip(MatrixStack matrices, ItemStack stack, int mouseX, int mouseY) {
		...
		List<ITextComponent> tooltip = this.getTooltipFromItem(stack);
		if (hoveredSlot != null) {
			appendDankInfo(tooltip,stack);
		}
		//crashes with nosuchmethod for some reason in production so lets call the forge patch directly instead
		//super.renderTooltip(matrices,tooltip, mouseX, mouseY, font == null ? this.font : font);
		net.minecraftforge.fml.client.gui.GuiUtils.drawHoveringText(matrices, tooltip, mouseX, mouseY, width, height, -1, font == null ? this.font : font);
